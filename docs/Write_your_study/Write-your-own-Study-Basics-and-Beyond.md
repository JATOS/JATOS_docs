...
These input fields accept JSON. The data you enter there becomes available in your study's JavaScript via `jatos.studyInput`, `jatos.componentInput`, and `jatos.batchInput` (still supported are the deprecated `jatos.studyJsonInput`, `jatos.componentJsonInput`, and `jatos.batchJsonInput`).
...
You can access these fields in your JavaScript with `jatos.studyInput.numberOfTrials`, `jatos.studyInput.retries`, and `jatos.studyInput.order`.
