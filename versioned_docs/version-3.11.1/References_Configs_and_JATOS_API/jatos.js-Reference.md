...
`jatos.studyInput` is the prefered way to get the study input and `jatos.studyJsonInput` is deprecated. Apart from that both contain the study input data you entered in the study properties. They are `{}` if the field was left empty.
...
`jatos.componentInput` is the prefered way to get the component input and `jatos.componentJsonInput` is deprecated. Apart from that both contain the component input data you entered in the component properties. They are `{}` if the field was left empty.
...
```javascript
jatos.setHeartbeatPeriod(30000); // Sets to a heartbeat every 30 seconds
```
...
Aborts the study and redirects to the given URL. This is useful if you have your own abort page. All previously submitted data will be deleted. Data stored in the Batch Session or Group Session are unaffected by this. It offers callbacks to signal success or failure in the ending.
...
`jatos.batchInput` is the prefered way to get the batch input and `jatos.batchJsonInput` is deprecated. Apart from that both contain the batch input data you entered in the batch properties. They are `{}` if the field was left empty.
...
Accessing the Batch Session is done via [JSON Patches (RFC 6902)](https://tools.ietf.org/html/rfc6902) and [JSON Pointer (RFC 6901)](https://tools.ietf.org/html/rfc6901). An introduction can be found under [jsonpatch.com](http://jsonpatch.com/).
...
Accessing the Group Session is done via [JSON Patches (RFC 6902)](https://tools.ietf.org/html/rfc6902) and [JSON Pointer (RFC 6901)](https://tools.ietf.org/html/rfc6901). An introduction can be found under [jsonpatch.com](http://jsonpatch.com/).
...
A convenience function for `jatos.groupSession.add`. Instead of a JSON Pointer path, it accepts the name of the field to be stored (without the slash in front). Therefore, it works only on the first level of the Group Session's object tree. If the name already exists in the Group Session, the value will be overwritten. If the group session write scope is activated the field name must be the group member ID or `shared`.
...
Replaces the whole session data. If the replacing object is rather large, it might be better, performance-wise, to replace only individual paths. Each session writing involves sending the changes in the session via a JSON Patch to the JATOS server. If the session is large, this data transfer can take some time. In this case, use other session functions, like `set`, `add`, or `replace`. Function `setAll` cannot be used, if the group session write scope is activated.
...
Clears the entire Group Session data and sets it to an empty object `{}`. Function `clear` cannot be used, if the group session write scope is activated.
...
10. If the group session write scope is activated, you may only write to within the subtree of `/<group-member-id>` or `/shared`. E.g. if the ID is `123`:

    ```javascript
    jatos.groupSession.add("/123", {})
       .then(() => console.log("Group Session was successfully updated"))
       .catch(() => console.log("Group Session synchronization failed"));
    ```
...
JSON Patch remove operation: Removes a value from an object or array (see [jsonpatch.com](http://jsonpatch.com/)). If the group session write scope is activated the path may only remove from its own `/<group-member-id>` (equal to study result ID) subtree or the common `/shared/` subtree.
...
3. If the group session write scope is activated, you may only write to within the subtree of `/<group-member-id>` or `/shared`. E.g. if the ID is `123` and the Group Session is `{"123": {"a": 100, "b": 101}}`:

    ```javascript
    jatos.groupSession.remove("/123/a")
       .then(() => console.log("Group Session was successfully updated"))
       .catch(() => console.log("Group Session synchronization failed"));
    ```

    Then, after the Group Session is successfully updated, the new object is `{"123": {"b": 101}}`.

-----

### `jatos.groupSession.replace`

JSON Patch replace operation: Replaces a value. This is equivalent to a "remove" operation followed by an "add" operation (see [jsonpatch.com](http://jsonpatch.com/)). If the group session write scope is activated the path may only write to its own `/<group-member-id>` (equal to study result ID) subtree or the common `/shared/` subtree.
...
3. If the group session write scope is activated, you may only write to within the subtree of `/<group-member-id>` or `/shared`. E.g. if the ID is `123` and the Group Session is `{"123": {"a": 100, "b": 101}}`:
    ...
    ```javascript
    jatos.groupSession.replace("/123/a", 999)
       .then(() => console.log("Group Session was successfully updated"))
       .catch(() => console.log("Group Session synchronization failed"));
    ```

    Then, after the Group Session is successfully updated, the new object is `{"123": {"a": 999, "b": 101}}`.

-----

### `jatos.groupSession.copy`

JSON Patch copy operation: Copies a value from one location to another within the JSON document. Both `from` and `path` are JSON Pointers (see [jsonpatch.com](http://jsonpatch.com/)). If the group session write scope is activated the target path may only write to its own `/<group-member-id>` (equal to study result ID) subtree or the common `/shared/` subtree; The origin path is unrestricted.
...
3. If the group session write scope is activated, you may only write to within the subtree of `/<group-member-id>` or `/shared`. E.g. if the ID is `123` and the Group Session is `{"123": {"a": 100}}`:

    ```javascript
    jatos.groupSession.copy("/123/a", "/123/b")
       .then(() => console.log("Group Session was successfully updated"))
       .catch(() => console.log("Group Session synchronization failed"));
    ```

    Then, after the Group Session is successfully updated, the new object is `{"123": {"a": 100, "b": 100}}`.

-----

### `jatos.groupSession.move`

JSON Patch move operation: Moves a value from one location to another. Both `from` and `path` are JSON Pointers (see [jsonpatch.com](http://jsonpatch.com/)). If the group session write scope is activated both pathes (origin and target) may only point to its own `/<group-member-id>` (equal to study result ID) subtree or the common `/shared/` subtree.
...
3. If the group session write scope is activated, you may only write to within the subtree of `/<group-member-id>` or `/shared`. E.g. if the ID is `123` and the Group Session is `{"123": {"a": 100}}`:
    ...
    ```javascript
    jatos.groupSession.move("/123/a", "/123/b")
       .then(() => console.log("Group Session was successfully updated"))
       .catch(() => console.log("Group Session synchronization failed"));
    ```

    Then, after the Group Session is successfully updated, the new object is `{"123": {"b": 100}}`.
