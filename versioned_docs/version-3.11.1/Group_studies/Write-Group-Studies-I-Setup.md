...
slug: /Write-Group-Studies-I-Setup
sidebar_position: 2
---

### How to Turn a Study into a Group Study

Go to your study, open **Properties**, and select the **Group study** checkbox.

![Group's property](/img/v311x/study-properties-group-study.png)

### Group Session Write Scope

If the group study is activated, you can then configure the group session write scope:

* If the checkbox is not selected, every group member can write anywhere in the group session data and could potentially overwrite data written by other members.
* If the checkbox is selected, each member can write only within their own scope, beginning with `/<group-member-id>` (equals study result ID), or within the shared scope at `/shared`.
...
Go to your study's "Study links" page. Group setup is done on a batch level. Click "Properties. There are three key properties for group studies:
