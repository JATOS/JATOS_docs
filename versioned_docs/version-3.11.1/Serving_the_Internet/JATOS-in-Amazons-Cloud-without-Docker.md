---
title: JATOS on AWS
slug: /JATOS-in-Amazons-Cloud-without-Docker
sidebar_position: 5
---

This page provides additional information on how to install JATOS on an AWS server. General installation advice can be found in [JATOS on a server](JATOS-on-a-server) and applies here as well. We recommend using JATOS together with a reverse proxy. See our instructions for [Apache](/JATOS-with-Apache) or [Nginx](/JATOS-with-Nginx). If you are looking for an easier way to install JATOS in the cloud, check out the tutorial [JATOS on DigitalOcean](JATOS-on-DigitalOcean).
...
7. If you **do not** use a reverse proxy like [Nginx](/JATOS-with-Nginx) or [Apache](/JATOS-with-Apache), configure the IP and port in `conf/jatos.conf`:  
...
9. (Optional) [Set up JATOS to auto-start](/JATOS-on-a-server#optional-auto-start-jatos-via-systemd).
